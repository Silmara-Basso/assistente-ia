...
index_name = 'sil-index'
index = Chroma.from_documents(sil_doc, gerador_embeddings, collection_name = index_name)

# Define a função de busca por similaridade
def sil_busca_similaridade(query, k = 2):
    matching_results = index.similarity_search(query, k)
    return matching_results

# Cria instância do LLM
llm_sil = OpenAI(openai_api_key = os.environ['OPENAI_API_KEY'], temperature = 0.3)

# Cria a chain para sistema de QA (Perguntas e Respostas)
chain = load_qa_chain(llm_sil, chain_type = 'stuff')

# Define a função para obter resposta
def sil_obtem_resposta(query):
    doc_search = sil_busca_similaridade(query)
...
@app.route('/silllm', methods=['POST'])
def silllm():
    query = request.form['query']
    response = sil_obtem_resposta(query)
